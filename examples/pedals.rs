...
fn main() {
    ...
    let idx: Vec<_> = names
        .iter()
        .map(|n| rat.unknown_named(n).unwrap())
        .collect();
    ...
    for d in [0.0, 0.5, 1.0] {
        let lo = meas(
            &rat,
            &[(rodent::DISTORTION, d), (rodent::FILTER, 0.0)],
            1000.0,
            1e-4,
        );
        let hi = meas(
        ...
        let big = meas(
            &rat,
            &[(rodent::DISTORTION, d), (rodent::FILTER, 0.3)],
            220.0,
            0.122,
        );
        println!(
            "  dist {d}: 1k gain {:.1} dB, 8k {:.1} dB | 0.122 V THD {:.1}% level {:.1} dB",
            lo.gain_db(),
            hi.gain_db(),
            big.thd_percent(),
            big.gain_db()
    ...
    for hz in [200.0, 1000.0, 3000.0, 10000.0] {
        ...
        println!(
            "  op-amp out, full distortion, {hz} Hz: {:.1} dB (ideal 67)",
            m.gain_db()
        );
    }
}
