...
fn run(label: &str, settings: Settings, reference: Option<f64>) -> f64 {
    ...
    let level = reference
        .map(|r| format!("{:+6.1} dB", 20.0 * (rms / r).log10()))
...
fn main() {
    ...
    for (gain, power_amp, name) in [
        ...
        run(
            &format!("{name}: legacy Off (resistor DI)"),
            base,
            Some(reference),
        );
        run(
            &format!("{name}: Brit V30 4x12, Dynamic 57"),
            Settings {
                acoustic: physical(&CabinetProfile::BRIT_V30),
                ..base
            },
            Some(reference),
        );
        ...
        run(
            &format!("{name}: American Open 2x12, Dynamic 57"),
            Settings {
    ...
    for (label, b) in [
        ("stage alone, one mic", MicSlot::Off),
        (
            "stage alone, two mics",
        ...
        println!(
            "{label:<46}{load:>7.2}%  ({:.0} ns/sample){}",
            load / 100.0 / RATE * 1e9,
            if acc.is_finite() { "" } else { " NaN" }
        );
    }
}
