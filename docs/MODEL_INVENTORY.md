...
| `amp_roland_jc120` | Jazz 120 | Roland **JC-120 Jazz Chorus**, CH-1, JC-120UT/JT Sep. 2000 | `jazz120.rs` | PARTIALLY IMPLEMENTED 2026-09-22: CH-1 complete, jacks to CN3 (two 2SK184 common-source stages, the passive 250k/250k/10k stack with its two slope caps, 1 M Volume and bright network, 2SC1815 follower). CH-2, reverb, BBD chorus and the two power amplifiers outstanding | [docs/models/jazz_120.md](models/jazz_120.md): Roland's own 1334 ppi service notes, plus three earlier editions kept because **CH-1 differs in every generation** | **none** -- the amplifier's two 60 W solid-state power amps are not built, so `Matched` resolves to nothing and the channel hands straight to the tone section. The first voice in the catalogue with no power stage | Roland 30-103D-3 in a 2x12 open back -- not built; the preset stands in with American Open 2x12 and the C12N | Jazz Clean | appended to `circuit`; `Gain::ALL` appended behind it, so `CALIBRATION` and `POWER_TRIM_DB` gained one row each | Jazz 120 |
