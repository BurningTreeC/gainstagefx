...
/// A long list shown one entry at a time: the chosen name between a back and a
/// forward mark. Click the left quarter to step back and anywhere else to step
/// forward; the wheel steps as well.
///
/// The model lists (twelve cabinets, nine speakers, thirteen microphones) would
/// each need several rows of segments. The panel reads top to bottom in signal
/// order, and a cabinet section several times the height of the amplifier it
/// follows would bury that order.
pub struct Stepper {
    param: ParamWidgetBase,
    names: Vec<&'static str>,
    /// Entries before this one are not offered (Mic A has no Off).
    first: usize,
    enabled: bool,
}
...
impl Stepper {
        ...
        .build(
            ...
            ParamWidgetBase::build_view(params, params_to_param, move |cx, data| {
                ...
                Label::new(
                    cx,
                    value.map(move |v| {
                        let index = ((v * (total - 1) as f32).round() as usize)
                            .clamp(first, total - 1);
    ...
    fn current(&self) -> usize {
        ...
        let v = self.param.unmodulated_normalized_value();
        ((v * (total - 1) as f32).round() as usize).clamp(self.first, total - 1)
    }

    fn step(&self, cx: &mut EventContext, delta: i64) {
        let total = self.names.len().max(1);
        let last = (total - 1) as i64;
        let next = (self.current() as i64 + delta).clamp(self.first as i64, last) as usize;
        let normalised = if total > 1 {
            next as f32 / (total - 1) as f32
        } else {
            0.0
        };
        self.param.begin_set_parameter(cx);
...
impl View for Stepper {
    ...
    fn event(&mut self, cx: &mut EventContext, event: &mut Event) {
        ...
        event.map(|window: &WindowEvent, meta| match window {
            WindowEvent::MouseDown(MouseButton::Left) => {
                let x = cx.mouse().cursorx - b.x;
                self.step(cx, if x < b.w / 4.0 { -1 } else { 1 });
                meta.consume();
            }
            WindowEvent::MouseScroll(_, y) => {
    ...
    fn draw(&self, cx: &mut DrawContext, canvas: &mut Canvas) {
        ...
        let scale = cx.scale_factor();
        let mut cell = vg::Path::new();
        cell.rounded_rect(b.x + 1.5 * scale, b.y, b.w - 3.0 * scale, b.h, 3.0 * scale);
        canvas.fill_path(
            &cell,
            &vg::Paint::color(rgba(GLOW, if self.enabled { 0.14 } else { 0.04 })),
        );
        canvas.stroke_path(
            &cell,
            &vg::Paint::color(rgba(0xffffff, if self.enabled { 0.16 } else { 0.06 }))
                .with_line_width(scale),
        );
    }
}
